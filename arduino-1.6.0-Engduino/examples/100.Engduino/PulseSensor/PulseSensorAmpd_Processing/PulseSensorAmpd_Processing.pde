...
//Parameters for Serial Events
String inData = new String("                    ");         // a string to hold incoming data
boolean stringComplete = false;  // whether the string is complete

boolean processingSerial = false;

//int sn = 0;

void setup() {
  size(700, 600);  // Stage size
  frameRate(100);  // per second
...
void draw() {
  ...
  if (Sensor > 0) {
    ...
    RawY[RawY.length-1] = RawY[RawY.length-2];
